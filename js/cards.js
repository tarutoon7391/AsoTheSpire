...
// 報酬抽選で使うカードプール
const REWARD_POOLS = {
  common: ["whirlwind", "pommelStrike", "ironWall", "anger", "battleTrance", "cleave"],
  uncommon: ["headbutt", "bloodForBlood", "combustStrike", "offering", "secondWind", "endure", "feed"],
  rare: ["immolate", "demonForm", "barricade"]
};
...
// カードインスタンスを作成する
function createCardInstance(id, upgraded = false) {
  return {
    id,
    upgraded
  };
...
window.REWARD_POOLS = REWARD_POOLS;
window.CARD_UPGRADE_LIBRARY = CARD_UPGRADE_LIBRARY;
window.createCardInstance = createCardInstance;
window.createStarterDeck = createStarterDeck;
